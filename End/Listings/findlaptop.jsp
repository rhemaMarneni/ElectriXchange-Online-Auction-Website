...
<body>
	...
	<h1 style="background-color: white; font-family: 'Trebuchet MS', sans-serif;">Laptop Auctions Found:</h1>
		<%
			...
			if(request.getParameter("sort").equals("priceHighToLow")){
				query = "select * from auctions a natural join " + 
						"(select * from items i natural join laptops p where i.item_id = p.item_id) p " +
						...
						" and charger = " + charger + " and touchscreen = " + touch + " and current_price <= " + price + " order by current_price desc";
			}
			else{
				query = "select * from auctions a natural join " + 
						"(select * from items i natural join laptops p where i.item_id = p.item_id) p " +
						...
						"item_condition = " + condition + " and brand = " + brand + " and color = " + color +
						" and ram = " + ram + " and os = " + os + " and storagetype = " + storage +
						" and charger = " + charger + " and touchscreen = " + touch + " and current_price <= " + price + " order by current_price asc";
			}
			...
			<table>
				...
				<%while(rs.next()){ %>
					<tr>
						...
						<td>$<%= rs.getString("current_price")%></td>
						<td><%= rs.getString("end_date")%></td>
						<td><input type="submit" value="Bid" class="button"></td>						
					</tr>
				<%} %>
			</table>
		</form>
		...
		<%} else{%>
			<div style="text-align: center">
				...
				<a href="searchlaptop.jsp" class='button' style="margin-left:46%;">Try Again</a>
			</div>
		<%}%>
</body>
</html>