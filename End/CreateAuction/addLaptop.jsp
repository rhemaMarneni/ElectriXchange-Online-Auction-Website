...
<body>
	...
	<div class="container" style="text-align: left;">
		<%
			...
			if(endDate.equals("")){
				out.println("Auction End Date Invalid. Try Again<br><br><a href='createLaptop.jsp' class='button'>Try Again</a>");
			}
			else{
				...
				if(minPrice.equals("0") || minPrice.equals("")){
					out.println("The minimum price must be above $1<br><br><a href='createlaptop.jsp' class='button'>Try Again</a>");
				}
				else if(!validDate){
					out.println("Auction End Date Invalid. Try Again<br><br><a href='createlaptop.jsp' class='button'>Try Again</a>");
				}
					...
					String query = "select * from items natural left join laptops where name = ? and item_condition = ? and manufacturing_location = ? and brand = ? and color = ? and "+
					"ram = ? and os = ? and storagetype = ? and charger = ? and touchscreen = ? and dimensions = ?";
					...
					if(rs.next()){
						...
						String addLaptopQuery = "insert into laptops values(?, ?, ?, ?)";
						PreparedStatement addLaptopStatement = con.prepareStatement(addLaptopQuery);
						addLaptopStatement.setInt(1, newItemID);
						addLaptopStatement.setString(2, touch);
						addLaptopStatement.setString(3, charger);
						addLaptopStatement.setString(4, dim);	
						
						addLaptopStatement.executeUpdate();
